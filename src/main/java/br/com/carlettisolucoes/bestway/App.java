...
public class App {
	...
	private synchronized void populateBestLength(Way way, int current, List<Way> ways) {
		int newLength = (current + way.factor);
		if (way.length > 0 && way.length < newLength) {
			return;
		}
		System.out.println("Current: " + way);
		// writeHtml(way);
		way.length = newLength;
